...
> `#AI Image Generation` `#Flat Design` `#Image Stylization` `#提示工程` 
> Pureza is a prompt set for generating flat, minimalist, solid-color AI images. It transforms complex images into simplified color block representations, emphasizing 'express through color blocks, simplify the complex.' Optimized for GPT Image 2/2.5 series models, best applied during post-processing stages for refined color accuracy.
...
> `#FinTech` `#Image API` `#JSON Config` `#Payment Systems` `#bank` `#bank-bin` `#bank-card` `#bank-code` `#bank-logo` 
> A bank logo repository enabling logo retrieval by bank codes. It offers multiple access methods including Alipay image API and self-hosted transparent PNG endpoints, with a comprehensive JSON mapping of bank codes to Chinese names. Ideal for payment systems, e-commerce checkout, and financial application development.
> <sub>bank.logo</sub>
...
> `#AI Agents` `#Memory Management` `#Persistent Storage` `#Semantic Retrieval` 
> FlyMemory is a persistent memory layer for personal AI agents, using sentence-level chunked storage with dual semantic+lexical retrieval. It features semantic deduplication, power-law temporal decay, and model-driven supersession (stale facts marked, not deleted). MCP server with automatic UserPromptSubmit hook enables zero-intrusion integration for coding agents.
...
> `#Docker Deployment` `#Emby Compatible` `#Home Entertainment` `#NAS Media Management` 
> MeBox is a self-hosted private media center built with Go and React, tailored for NAS and home entertainment. It features media library management, metadata scraping, full Emby/Jellyfin client protocol compatibility, remote Emby mounting, cloud STRM support, and multi-user permissions. Deploys via Docker with amd64/arm64 support.
...
> `#Go` `#Image Processing` `#QR Code Generator` `#WebAssembly` `#adjustable` `#cli` `#colorful` `#golang` `#halftone-qr-code` `#highly-customizable` `#library` `#qrcode` `#wasm` 
> go-qrcode is a pure Go QR code generation library supporting versions 1-40 with automatic version detection. It offers extensive customization including colors, cell shapes (rectangle/circle), border width, center logo, and WebAssembly support for browser-based generation. Also supports Halftone style output.
...
> `#Canvas LMS` `#EdTech` `#File Sync` `#Offline Access` 
> CanvasSync is a student-oriented synchronization tool for Canvas LMS that automatically downloads course modules, assignments, and files via the official API, mirroring the server's folder structure locally. It supports syncing files, HTML pages, external links, and assignment attachments with encrypted authentication storage. Ideal for offline access and backup of educational materials.
...
> `#API Integration` `#Automation Tool` `#Data Backup` `#EdTech` 
> A Python tool to export complete student data from Canvas LMS for personal backup before graduation or leaving an institution. Exports assignments, submissions, announcements, discussions, pages, files, and modules into organized JSON files by term and course. Optional HTML snapshots enable offline viewing of course content.
...
> `#AI Assistant Extension` `#Context Management` `#Interactive TUI` `#Skill Management` 
> pi-skill-toggle is an extension for the pi AI assistant that solves context pollution from skill hoarding. It provides an interactive TUI to toggle skills between enabled, hidden (manual invocation only), and fully disabled states. Uses pi's built-in resource filtering to exclude disabled skills from the system prompt while keeping them visible for future use.
...
> `#Content Aggregation` `#Image Search Engine` `#Multi-Platform Crawler` `#Telegram Bot` `#anime` `#crawler` `#danbooru` `#image-viewer` `#kawaii` `#nhentai` `#picture` `#pixiv` `#telegram` `#telegram-bot` `#waifu` 
> ManyACG is an anime artwork aggregation platform that crawls and organizes content from multiple sources including Pixiv, Twitter, Danbooru, and Bilibili. It automates Telegram channel management via bot, and can construct a standalone image sharing website from stored data. Features include image hashing deduplication, reverse image search, AI tagging, and MeiliSearch-powered hybrid search, with flexible storage options.
...
> `#Network Proxy` `#Privacy Enhancement` `#Rule Engine` `#Rust` 
> meow-rs is a high-performance proxy kernel in Rust, compatible with mihomo/Clash Meta. It supports Shadowsocks, Trojan, VLESS, VMess, Hysteria2 protocols with privacy features like ECH and uTLS fingerprinting. Features domain/IP/GeoIP/ASN-based rule engine and proxy groups with load balancing and health checking.
...
> `#ACG Culture` `#Anime Lore` `#Japanese Anime` `#Light Novel Adaptation` 
> Comprehensive repository for A Certain Scientific Railgun, covering the manga's complete 170 chapters, four anime seasons, and character profiles. Details the Academy City's setting, protagonist Misaka Mikoto, story arcs, and anime production. Includes cultural notes on Bilibili's origin and the character's legendary popularity in light novel rankings.
...
> `#AI Agents` `#Cross-Platform` `#Productivity Tool` `#提示工程` `#浏览器插件` `#ai-studio` `#browser-extension` `#bun` `#chat-management` `#chatgpt` `#chrome-extension` `#claude-ai` `#deepseek` `#dsh` `#dsh-plugin` `#edge-addon` `#firefox-addons` `#gemini` `#safari-extension` 
...
> `#Cross-platform` `#Interactive Computing` `#Real-time Visualization` `#Scientific Computing` `#computational-science` `#gui` `#gui-application` `#interactive-visualizations` `#javascript-project` `#mathematica` `#notebook` `#plotting` `#ui` `#wolfram` `#wolfram-language` 
...
> `#Real-time Communication` `#Video Conferencing` `#WebRTC` `#私有化部署` `#collaboration` `#docker` `#file-sharing` `#jitsi-alternative` `#live-streaming` `#mediasoup` `#mirotalk` `#nodejs` `#open-source` `#real-time-communication` `#rtmp` `#screen-sharing` `#self-hosted` `#video-chat` `#video-conferencing` `#webinar` `#webrtc` `#webrtc-sfu` `#websocket` `#zoom-alternative` 
...
> `#Cloudflare` `#Image Storage` `#Static Site` `#无服务器` 
> A modern personal image hosting service built with Astro on Cloudflare, using R2 for storage and KV for metadata. Supports drag-drop, paste, and batch uploads with in-browser image editing, admin dashboard, API key authentication, anti-hotlinking, and custom domain configuration, deployable within Cloudflare's generous free tier.
...
> `#Cloud-Native` `#DevOps Automation` `#Kubernetes` `#Web Management Platform` `#admin-dashboard` `#cloud-native` `#dashboard` `#devops` `#gin` `#golang` `#helm` `#k8s` `#k8s-admin` `#k8s-cluster` `#k8s-deployment` `#k8s-install` `#k8s-mutli-cluster` `#k8s-stack` `#kubectl` `#kubernetes` `#multi-cluster` `#react` `#typescript` 
